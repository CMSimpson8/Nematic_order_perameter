...
%COPY THIS INTO CAMMAND BAR:
%im = import_tif; % select a tif image, should be a single image or projection, not a z-stack
%out = calculate_order_parameter(im, 'plot', 'yes');

%For a folder with many images:
%ims_BJ_MPA_1 = import_tif_sequence;
%for i=1:size(ims_BJ_MPA_1,3); out_MPA_1(i) = calculate_order_parameter(ims_BJ_MPA_1(:,:,i)); end; % loop over all images
